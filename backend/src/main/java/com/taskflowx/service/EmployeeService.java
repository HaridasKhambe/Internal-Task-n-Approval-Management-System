...
public class EmployeeService {
    ...
    @Transactional
    public TaskResponse updateTaskStatus(Long taskId, UpdateStatusRequest request, String employeeEmail) {
        Task task = taskService.getTaskById(taskId);
        User employee = userRepository.findByEmail(employeeEmail)
                .orElseThrow(() -> new ResourceNotFoundException("Employee not found"));
        ...
        if (requestedStatus != TaskStatus.IN_PROGRESS && requestedStatus != TaskStatus.SUBMITTED) {
            throw new IllegalArgumentException("Employees can only update status to IN_PROGRESS or SUBMITTED");
        }
